    ...
    try:
        ...
        def check_dongTien(sym):
            try:
                ...
                # DK2: Giá > 5 (nghìn đồng — giá FireAnt tính bằng đồng nên > 5000)
                price = float(c[i])
                if price < 5000: return None

                # DK3: Volume > 80% trung bình 10 phiên gần nhất
                ma_vol10 = np.mean(v[max(0,i-10):i]) if i > 0 else v[i]
                if ma_vol10 == 0: return None
                if vol < 0.8 * ma_vol10: return None

                # DK4: Giá tăng > 2% so với phiên trước
                if i < 1: return None
                prev_close = float(c[i-1])
                if prev_close == 0: return None
                pct = (price - prev_close) / prev_close * 100
                if pct < 2.0: return None

                # DK6: Giá > 102% MA5
                ma5 = sma(c, 5)
                if np.isnan(ma5[i]): return None
                if price < 1.02 * ma5[i]: return None
                ...
                return {
                    ...
                    'pct':       round(pct, 2),
                    'volume':    int(vol),
                    'vol_ratio': vol_ratio,  # Volume so với TB10 (>1 = cao hơn TB)
                    'ma5':       round(float(ma5[i]), 2),
                    'above_ma5_pct': round((price - ma5[i]) / ma5[i] * 100, 2),
                }
            except: return None
...
def acc_scan():
    try:
        ...
        def calc_rsi(c, n=14):
            ...
            ag = np.mean(gains[1:n+1]); al = np.mean(losses[1:n+1])
        ...
        def check_acc(sym):
            try:
                ...
                # ── 1. VPA signals trong 5 ngày gần nhất (+2) ──
                vpa_found = []
                for i in range(max(5, n-5), n):
                    va = vol20[i] if not np.isnan(vol20[i]) else np.mean(v[max(0,i-5):i+1])
                    sa = sp20[i]  if not np.isnan(sp20[i])  else np.mean(spread[max(0,i-5):i+1])
                    # NoSupply
                    if c[i]<c[i-1] and spread[i]<sa*0.9 and v[i]<va*0.9 and c[i]>(l[i]+spread[i]*0.3):
                        vpa_found.append('NoSupply')
                    # Test
                    if v[i]<va*0.8 and l[i]<=min(l[max(0,i-3):i]) and c[i]>(l[i]+spread[i]*0.5):
                        vpa_found.append('Test')
                    # StopVol
                    if v[i]>va*1.2 and c[i]>(l[i]+spread[i]*0.5) and l[i]<=min(l[max(0,i-5):i+1]):
                        vpa_found.append('StopVol')
                if vpa_found:
                    score += 2
                    reasons.append('VPA:'+','.join(set(vpa_found)))

                # ── 2. Volume giảm dần trong 10 ngày (+2) ──
                if not np.isnan(vol20[-1]):
                    vol_ratio = np.mean(v[-10:]) / vol20[-1]
                    if vol_ratio < 0.75:
                        score += 2
                        reasons.append('Vol giam '+str(round((1-vol_ratio)*100))+'%')
                    elif vol_ratio < 0.9:
                        score += 1
                        reasons.append('Vol giam nhe')

                # ── 3. Giá đi ngang biên độ hẹp 10-20 ngày (+2) ──
                look = min(20, n)
                hi = max(c[-look:]); lo = min(c[-look:])
                rng = (hi-lo)/lo*100 if lo > 0 else 99
                if rng < 5:
                    score += 2
                    reasons.append('Bien do hep '+str(round(rng,1))+'%')
                elif rng < 8:
                    score += 1
                    reasons.append('Bien do vua '+str(round(rng,1))+'%')

                # ── 4. Higher Low (+2) ──
                # Compare recent low vs low 20 bars ago
                recent_low  = min(l[-5:])
                prev_low    = min(l[-25:-5]) if len(l) >= 25 else min(l[:-5])
                if recent_low > prev_low * 1.01:
                    score += 2
                    reasons.append('Higher Low')
                elif recent_low > prev_low * 0.99:
                    score += 1
                    reasons.append('Low on dinh')

                # ── 5. MA20 flatten + giá trên MA20 (+1) ──
                if not np.isnan(ma20[-1]) and not np.isnan(ma20[-5]):
                    ma20_slope = (ma20[-1] - ma20[-5]) / ma20[-5] * 100
                    if abs(ma20_slope) < 1.5 and c[-1] >= ma20[-1]:
                        score += 1
                        reasons.append('MA20 flat+above')
                    elif c[-1] > ma20[-1] and not np.isnan(ma50[-1]) and ma20[-1] > ma50[-1]:
                        score += 1
                        reasons.append('Tren MA20+MA50')

                # ── 6. RSI 40-60 (+1) ──
                rsi = calc_rsi(c[-30:])
                if 38 <= rsi <= 62:
                    score += 1
                    reasons.append('RSI '+str(round(rsi,0))+' (trung tinh)')

                if score < 6: return None

                pct = (c[-1]-c[-2])/c[-2]*100 if n>1 else 0
                phase = 'SAP BREAKOUT' if score >= 8 else 'TICH LUY TOT'
                ...
                return {
                    ...
                    'range_pct': round(float(rng),1),
                }
            except: return None

        results = []
        with ThreadPoolExecutor(max_workers=30) as ex:
            futures = {ex.submit(check_acc, s): s for s in BREADTH_SYMBOLS}
            for f in as_completed(futures):
                r = f.result()
                if r: results.append(r)
